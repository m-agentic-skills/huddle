...
<step-policy step="discussion">
  ...
  <state-rules>
    ...
    <rule>The canonical shape of huddle-state.json is defined in `references/huddle-state-schema.xml`. Writers (Elango, personas, Sreyash wrap) must follow that schema. Cross-refs (linked_issues, challenges.targets, evidence.linked_decisions, open_questions.targets) must resolve to real ids in the same state. Do not fabricate empty arrays into populated shapes — leave empty if nothing in rationale supports them.</rule>
